...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://pixelslices.cdn.prismic.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"page","path":"/:uid"}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://pixelslices.cdn.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=pixelslices&new=true'
